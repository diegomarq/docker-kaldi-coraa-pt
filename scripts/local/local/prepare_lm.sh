...
cat data/valid_train/text data/valid_test/text |\
 cut -f2- -d' ' |\
  sed -e 's:[ ]\+: :g' |\
   sort -u > $locdata/corpus.txt
...
echo "scanning $locdata/corpus.txt ..."
for word in $(cat $locdata/corpus.txt | dos2unix)
do
  echo $word >> wlist.tmp 
done 

cat wlist.tmp | sort -u | uniq > $locdata/vocab-full.txt
