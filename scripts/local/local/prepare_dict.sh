...
#  sed -e 's/[,.;<>:?!1234"“567890()@%]/ /g' | tr '[A-Z]' '[a-z]' | tr "\'" " " > $locdict/cmudict-plain.txt
...
cat $locdata/vocab-full.txt >> $locdict/vocab-oov.txt

java -jar $HOME/kaldi/nlp-generator-master/fb_nlplib.jar -g -i $locdict/vocab-oov.txt -o $locdict/lexicon-oov.tmp

# words first column in lexicon-oov to words of first column in vocab-oov
awk 'FNR==NR{a[NR]=$1;next}{$1=a[FNR]}1' $locdict/vocab-oov.txt $locdict/lexicon-oov.tmp > $locdict/lexicon-oov.txt

cat $locdict/lexicon-oov.txt | sort > $locdict/lexicon.txt
