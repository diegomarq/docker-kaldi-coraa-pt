...
PYTHONPATH=${PYTHONPATH:-}:$SEQUITUR python setup.py install --prefix "$SEQUITUR" --install-lib "$SEQUITUR"
) || {
...
site_packages_dir=$(cd sequitur-g2p; find ./lib{,64} -name "sequitur*.egg" | head -n 1 | xargs dirname)
(
  set +u
  [ ! -z "${SEQUITUR}" ] && \
